...
exp_args = dict(
    ...
    learn=dict(
        device='cuda:0', local_eps=5, global_eps=300, batch_size=100, optimizer=dict(name='sgd', lr=0.1, momentum=0.9), test_place=['after_aggregation', 'before_aggregation']
    ),
    model=dict(
        name='resnet10',
        input_channel=3,
    ...
    other=dict(test_freq=1, logging_path=f'./logging/cifar100_separate_resnet/{args.noniid}_{args.alpha}_{args.seed}')
)
